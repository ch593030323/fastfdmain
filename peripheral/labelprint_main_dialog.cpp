    ...
    case 0://POSTEK Q8/200
    {
        ...
        for(int rr = 0, rr_cout = model->rowCount(),actual_row = 0; rr < rr_cout; rr++) {
            for(int actual_r = 0, actual_count = model->data(model->index(rr, 0)).toInt()
                ...
                vvs[rrr][cc]= print_piece_POSTEK(model, ssrmap, printer0, rr,piece_width * (actual_row % column));
            }
    ...
    case 1://TSC TTP-244 PLUS"
    {
        ...
        for(int rr = 0, rr_cout = model->rowCount(),actual_row = 0; rr < rr_cout; rr++) {
            for(int actual_r = 0, actual_count = model->item(rr, 0)->text().toInt()
                ...
                int rrr=actual_row/column;
                if(cc==0)vvs.append(QVector<QString>(column));
                vvs[rrr][cc]= print_piece_TSC(model, ssrmap, printer0, rr,piece_width * (actual_row % column));
    ...
    case 2://SC2100(商超专用)
    {
        ...
        for(int rr = 0, rr_cout = model->rowCount(),actual_row = 0; rr < rr_cout; rr++) {
            for(int actual_r = 0, actual_count = model->item(rr, 0)->text().toInt()
                ...
                int rrr=actual_row/column;
                if(cc==0)vvs.append(QVector<QString>(column));
                vvs[rrr][cc]= print_piece_ARGOX_PPLA(model, ssrmap, printer0, rr,piece_width * (actual_row % column));
            }
            if(istest) break;//若是测试则打印一次后，直接跳出
        }
        QString ins;
    ...
    case 3://ARGOX CP-2140
    {
        ...
        for(int rr = 0, rr_cout = model->rowCount(),actual_row = 0; rr < rr_cout; rr++) {
            for(int actual_r = 0, actual_count = model->item(rr, 0)->text().toInt()
                ...
                int rrr=actual_row/column;
                if(cc==0)vvs.append(QVector<QString>(column));
                vvs[rrr][cc]= print_piece_ARGOX_PPLB (model, ssrmap, printer0, rr,piece_width * (actual_row % column));
            }
            if(istest) break;//若是测试则打印一次后，直接跳出
        }
        QString ins;
    ...
    case 7://佳博GP-9034T
    {
        ...
        for(int rr = 0, rr_cout = model->rowCount(),actual_row = 0; rr < rr_cout; rr++) {
            for(int actual_r = 0, actual_count = model->item(rr, 0)->text().toInt()
                ...
                int rrr=actual_row/column;
                if(cc==0)vvs.append(QVector<QString>(column));
                vvs[rrr][cc]= print_piece_GAINSCHA (model, ssrmap, printer0, rr,piece_width * (actual_row % column));
            }
            if(istest) break;//若是测试则打印一次后，直接跳出
        }
...
QString labelprint_main_dialog_object::print_piece_POSTEK(QStandardItemModel *model, const SSRPairMap &ssrmap, printer_label_postek &printer, int rr, int start_x)
{
    if(rr >= model->rowCount()) return "";//当前行数超过总行数

    //获取 打印机设置属性 ； 根据属性将ssrmap的内容转换成需要打格式:[dots 、200dpi]
    SSRPairMap map = ssrmapTranslate(ssrmap, "dots", "200DPI", labelprint_design_dialog::LEFTTOP);
    ...
    int wide_width = lds::sysconf->value(QString("label_print/wide_width"),"2").toInt();
    int narrow_width = lds::sysconf->value(QString("label_print/narrow_width"),"2").toInt();
    ...
    QString onebartype_key = lds::sysconf->value(QString("label_print/onebartype"), "Code 128 AUTO").toString();
    QString font_key = lds::sysconf->value(QString("label_print/font"), "FONT001").toString();

    QString onebartype = printer.print_bar_code_map().value(onebartype_key);
    QString font = printer.print_text_map().value(font_key);
...
QString labelprint_main_dialog_object::print_piece_TSC(QStandardItemModel *model, const SSRPairMap &ssrmap, printer_label_tsc_ttp_244_plus &printer, int rr, int start_x)
{
    if(rr >= model->rowCount()) return "";//当前行数超过总行数
...
QString labelprint_main_dialog_object::print_piece_ARGOX_PPLA(QStandardItemModel *model, const SSRPairMap &ssrmap, printer_label_argox_PPLA &printer, int rr, int start_x)
{
    if(rr >= model->rowCount()) return "";//当前行数超过总行数
...
QString labelprint_main_dialog_object::print_piece_ARGOX_PPLB(QStandardItemModel *model, const SSRPairMap &ssrmap, printer_label_argox_PPLB &printer, int rr, int start_x)
{
    if(rr >= model->rowCount()) return "";//当前行数超过总行数
    ...
    foreach(QString name, names) {
        ...
        int h = map[name].value(1).toRect().height();//高度
        bool istext=map[name].value(3).toBool();
        int rotate = map[name].value(4).toInt();//旋转
        switch(type) {
        case Shape::Rectangle:
            ins += printer.drawText(
                        textCutN(istext, key, text)
                       ...
                       ,  (printer_label_argox_PPLB::ROTATE)rotate//printer_label_argox_PPLB::ROTATE_0
                       ,  size.width()
                       ,  size.height()
                        );
            break;
...
QString labelprint_main_dialog_object::print_piece_GAINSCHA(QStandardItemModel *model, const SSRPairMap &ssrmap, printer_label_gainscha &printer, int rr, int start_x)
{
    if(rr >= model->rowCount()) return "";//当前行数超过总行数
...
QString labelprint_main_dialog_object::print_piece_HPRT(QStandardItemModel *model, const SSRPairMap &ssrmap, printer_label_tsc_HPRT &printer, int rr, int start_x)
{
    if(rr >= model->rowCount()) return "";//当前行数超过总行数
