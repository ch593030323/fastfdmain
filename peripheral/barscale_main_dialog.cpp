    ...
    down_isbreak(false)
{
    ...
    ui->spinBox_barcode_scaleflag->setValue(lds::sysconf->value("barscale/scaleflag", 21).toInt());
...
void barscale_main_dialog::goodsadd(const QList<BARSCAEL_ITEMS> items)
{
    ...
    foreach(BARSCAEL_ITEMS item, items) {
        ...
        downmodel0->setData(downmodel0->index(row, BARSCALE_FLAG), ui->spinBox_barcode_scaleflag->value());
    }
...
void barscale_main_dialog::save_barscale()
{
    ...
    lds::sysconf->setValue("barscale/scaleflag",ui->spinBox_barcode_scaleflag->value());
