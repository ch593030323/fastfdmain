                ...
                if(vch_plan_k3o3_state.mid(index, 1) == key) {//未厨打
                    ...
                    if(vch_plan_id.toInt() >= 10) {//厨打方案的编号不能小于10
                        ...
                        query_local.execInsert("table_kitchen", qrtVariantPairList()
                                               ...
                                               << qrtVariantPair("vch_print_memo", vch_print_memo.jsonToString())
...
bool w_bt_dish_kitchen_dish_thread_event::kitchen_update_transaction(lds_query_db &query_local, lds_query_db &query, int int_kitchen_section, int int_indexbox, int key_state)
{
    ...
    while(query_local.next()) {
        ...
        if(query.execUpdate(tablename,
                            qrtVariantPairList()
                            << qrtVariantPair("vch_plan_k3o3_state", getSqlReplaceIndex(vch_plan_k3o3_state, int_kitchen_section, w_bt_dish::k2o3_state2hex_char(w_bt_dish::KITCHEN_FINISH)))
                            << qrtVariantPair("dt_operdate", curdt),//打印时间
                            QString(" int_orderflow = %1 ").arg(int_flowid))) {
            continue;
        }
        query.db.rollback();
        return false;
    }
    query.execDelete("cey_u_orderdish_print_tr", QString(" date(dt_operdate)  <>  '%1' ").arg(curdt.toString("yyyy-MM-dd")));
    query.db.commit();
    return true;
}
...
BILL_CMB_HEADER:
            ...
            if(dishname_2.count() > 0) {
                ...
                if(vch_dishname.count() > 0) {
                    if(/*f_get_sysparm("kt_spinBox_vch_dishname_row", "1") == "1" && */f_get_sysparm("kt_checkBox_vch_dishname") == "1")print_format.drawText(bp_content_len, public_printby_ch_billno::getfixedlen(QString(lds::size_g_of_u(preapp), ' ') + vch_dishname,  kt_spinBox_vch_dishname_width, "0"), kt_spinBox_vch_dishname_width, Qt::AlignLeft);            print_format.drawN();
                    print_format.drawN();
                }
            }
...
QByteArray w_bt_dish_kitchen_dish_thread_event::kitchen_json_make(const w_bt_dish::kitchenState key_state, lds_query_db &query_local, int x, int int_indexbox, int vch_plan_id_TRY_count)
{
    ...
    if(query_local.next()) {
        ...
        cJSON_help::_cJSON_AddValueToObject(obj, "time", query_local.record().value("dt_operdate").toDateTime().toString(yyyyMMddhhmmss));
