...
interloopp0:
        ...
        vch_print_memo.from100110(vch_cook);
        ...
        if(query.execUpdate("cey_u_table", "num_cost", public_sql::getTotalBy_ch_billno(ch_billno), QString(" ch_billno = '%1' ").arg(ch_billno))) {
            ...
            if(fexpandmain_2::guqing_update(dishno, -dishnum_back_total)) {
                ...
                {//退菜单
                    ...
                    foreach(const num_backData &d, d_list) {
...
void w_sys_manage_wirelessordermonitoring_box::CLCC(int baseNo)
{
    ...
    if(!query.execUpdate("cey_u_orderdish_print_tr",
                         ...
                         QString("select a.int_orderflow,  '%1' as `vch_plan_k3o3_state`  from "
                                 "cey_u_orderdish_print_tr a,  cey_bt_dish b where "
...
bool w_sys_manage_wirelessordermonitoring_box::quitdish(QString &errstring, const QString &ch_billno, const QString &ch_dishno, double dishnum_back_total, const QString &vch_reason, const QString &operid, QList<num_backData> &d_list)
{
    ...
    while(dishnum_back_total  > 0 && query.next()) {
        ...
        if(!fexpandmain_2::static_toquit_1_suit(errstring, int_flowid, dishnum_back_total,vch_reason,operid, dt, d_list)) {
            goto failure;
        }
    }
    return true;
...
void w_sys_manage_wirelessordermonitoring_wx::basestationTAndroid(const QByteArray &request, const QHostAddress &host, quint16 port)
{
    ...
    if(readline.startsWith("_DXFD")) {//DXFD
        ...
        lds_query_hddpos::tran_saction();
        if(query.execUpdate("cey_u_orderdish_print_tr", "vch_plan_k3o3_state", QString (6, w_bt_dish::k2o3_state2hex_char(w_bt_dish::KITCHEN_CUICAI)),
        ...
        //获取单号
        lds_query_hddpos::tran_saction();
        if(query.execUpdate("cey_u_orderdish_print_tr a", "a.vch_plan_k3o3_state", QString (6, w_bt_dish::k2o3_state2hex_char(w_bt_dish::KITCHEN_CUICAI)),
        ...
        //打印点菜单
        ch_billno = lines.value(1);
        public_printby_ch_billno print_ch_billno(ch_billno, lds::get_soft_curent_language_first());
