...
bool w_bt_dish_base_dish_type::tosave()
{
    ...
    //        if(tablemodel->record(row, "ch_dish_typeno").toString().length() != 2) {
    //            lds_messagebox::warning(this, MESSAGE_TITLE_VOID, QObject::tr("大类编号必须为2位"));
    //            return false;
    //        }
    //    }
...
void w_bt_dish_base_new_virtual::toSetCountWeight(bool check)
{
    if(ui->checkBox_ch_weight == this->sender() && check) {
        if(0 == lds_messagebox::question(this, MESSAGE_TITLE_VOID, QObject::tr("启用称重计数"), QObject::tr("是"), QObject::tr("否"))) {
            ...
            dialog.ui->label->setText(QObject::tr("取重") + "(KG)");
            ...
            connect(&countprice,SIGNAL(weighComming(double)),dialog.ui->lineEdit,SLOT(setValue(double)));
            //
            if(QDialog::Accepted == transparentCenterDialog(&dialog).exec()) {
...
{
    if(imagedishdir.isEmpty() ) {
        lds_messagebox::warning(this, MESSAGE_TITLE_VOID, QObject::tr("图片(文件夹)为空"));
        return;
    }
    if(!QDir(imagedishdir).exists() ) {
        QDir().mkpath(imagedishdir);
    }
    if(!ui->lineEdit_ch_dishno->isEnabled()) {
        QString filepath = getFilePath::getOpenFileName(this, QObject::tr("选择图片"),
                                                #if defined(Q_OS_WIND) || defined(QT_DEBUG)
