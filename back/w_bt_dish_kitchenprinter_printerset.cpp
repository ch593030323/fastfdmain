...
void w_bt_dish_Kitchenprinter_printer_thread::emitsignalTrigger(const QString &bp_ch_billno, const QString &bp_languageKey, int label_flowid_greater_than, int pay_print_mode)
{
    //后台打印
    w_bt_dish_Kitchenprinter_printer_thread_event::mutex.lock();
    emit signalTrigger(bp_ch_billno, bp_languageKey, label_flowid_greater_than, pay_print_mode);
}

void w_bt_dish_Kitchenprinter_printer_thread::emitsignalTrigger(const QString &bp_ch_billno, const QString &bp_languageKey, int label_flowid_greater_than, int pay_print_mode, const QImage &img)
{
    //后台打印
    w_bt_dish_Kitchenprinter_printer_thread_event::mutex.lock();
    emit signalTrigger(bp_ch_billno, bp_languageKey, label_flowid_greater_than, pay_print_mode, img);
}
...
void w_bt_dish_Kitchenprinter_printer_thread::run()
{
    ...
    connect(this, SIGNAL(signalTrigger(QString,QString,int, int,QImage)), &event, SLOT(toprintBill(QString,QString,int, int,QImage)));
    this->exec();
}
...
void w_bt_dish_Kitchenprinter_printer_thread_event::toprintBill(const QString &bp_ch_billno, const QString &bp_languageKey, int label_flowid_greater_than, int pay_print_mode)
{
    public_printby_ch_billno printer(bp_ch_billno, bp_languageKey);
    printer.toPayPrint(printer.defpath(), n_func::f_get_sysparm("bp_count", "1").toInt(), label_flowid_greater_than, (public_printby_ch_billno::payPrintMode)pay_print_mode);
    w_bt_dish_Kitchenprinter_printer_thread_event::mutex.unlock();
}

void w_bt_dish_Kitchenprinter_printer_thread_event::toprintBill(const QString &bp_ch_billno, const QString &bp_languageKey, int label_flowid_greater_than, int pay_print_mode, const QImage &image)
{
    public_printby_ch_billno::payPrint(bp_ch_billno, bp_languageKey, label_flowid_greater_than, pay_print_mode, image);
    w_bt_dish_Kitchenprinter_printer_thread_event::mutex.unlock();
}
...
void w_bt_dish_Kitchenprinter_display_thread_event::toDisplay(const QByteArray &json)
{
    ...
    if(public_sql::display_led._type != lds::sysconf->value("system_setting/kexianleixing").toString()) {
        public_sql::display_led._type.assignValue(lds::sysconf->value("system_setting/kexianleixing").toString());
    }
