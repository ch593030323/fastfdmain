...
bool fexpandmain_model_sqltablemodel::can_change_price(int row, QString *errstring) const
{
    ...
    if(false == w_sys_manage_operatorright::operate_right_check_code("0816", lds::gs_operid)) {
...
double fexpandmain_model_sqltablemodel::pay_without_low_service(int tax_service_form_value, double consum, double present, double discount, double num_rate)
{
...
void num_backData::print() const
{
    ...
    qDebug() << "vch_reason" << vch_print_memo_remain.sqlValue();
#endif
}
...
double fexpandmain_keepData::pay_total(const fexpandmain_model_sqltablemodel_data *m) const
{
    ...
    int tax_service_form_value = m->get_cey_sys_parameter_value("tax_service_form_value", 0).toInt();
    double service = pay_service(ch_billno);
    double total =  fexpandmain_model_sqltablemodel::pay_without_low_service(tax_service_form_value, consum, present, discount,num_rate());
    ...
    if(tax_service_form_value == 1
            || tax_service_form_value == 3)
        pay_msg +=
