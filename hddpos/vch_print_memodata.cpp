...
QString vch_print_memoData::jsonToString(double num_x)
{
    QString desc;
    QString name;
    double price;
...
void vch_print_memoData::duplicate(const QString &no, double num)
{
...
void vch_print_memoData::from100110(const QString &str100110)
{
    clear();
    for(int k = 0; k < str100110.count(); k += 3) {
        duplicate(str100110.mid(k, 3));
    }
}
...
void vch_print_memoData::sub(const vch_print_memoData &other)
{
    for(int k = 0; k < count(); k++) {
