...
{
    QString name = qAppTr(w_sys_manage_operatorright::zh(code));
...
#endif
    if(!w_sys_manage_operatorright::operate_right_check_code(code, lds::gs_operid)) {
        lds_messagebox::warning(parent, MESSAGE_TITLE_VOID,
                                QObject::tr("操作员") + ":" + lds_query_hddpos::selectValue(QString(" SELECT vch_operator FROM hddpos.cey_sys_operator where vch_operID = '%1' ").arg(lds::gs_operid)).toString() + "(" + lds::gs_operid + ")\n" +
                                QObject::tr("操作员权限不足") + ":" + name + "(" + code + ")");
        return false;
    }
    return true;
}
...
{
    QString code = w_sys_manage_operatorright::code(zh);
    return operate_right_check_code(parent, code);
}
