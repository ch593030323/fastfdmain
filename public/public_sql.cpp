...
#ifdef Q_OS_WIN32
static void PUBLIC_MYSQL_INSTALL_PROCESS(QEventLoop *eloop, QObject *messageBar, bool *ret) {
    mysql_install_obj d;
    d.setMessageBar(messageBar);
    *ret = d.install();
    eloop->quit();
}

static bool PUBLIC_MYSQL_INSTALL(lds_messagebox_loading_show *messageBar) {
...
bool public_sql::CONNECT_MySQL(lds_messagebox_loading_show *loading)
{
    ...
    while(false ==db.open()) {
        ...
        if(public_sql::islocal) {
            if(PUBLIC_MYSQL_INSTALL(loading)) {
                continue;
            }
            public_sql::MySQL_info = public_sql::err_path_arg.arg(QObject::tr("操作失败")).arg(loading->text());
...
#endif
        loading->setText(QObject::tr("首次使用，正在还原数据库") + appendMsg);
#ifdef QT_SCPOS_SUPPORT
...
{
    QDir().mkpath(lds::localdataLocalePath);
    foreach(const QFileInfo &info_dir, QDir("userdata/settings/locale").entryInfoList(QDir::Dirs | QDir::NoDotAndDotDot)) {
        QString target_dir = lds::localdataLocalePath + "/" + info_dir.fileName();
        QDir().mkpath(target_dir);
        foreach(const QFileInfo &info_file, QDir(QString() + "userdata/settings/locale" + "/" + info_dir.fileName()).entryInfoList(QDir::Files | QDir::NoDotAndDotDot)) {
            if(false == QFile::exists(target_dir +"/" + info_file.fileName())) {
                QFile::copy(info_file.filePath(), target_dir + "/" + info_file.fileName());
            }
        }
    }
}
...
bool public_sql::syslimitcheck(QWidget *parent)
{
    ...
    forever{
        ...
        if(limit_day <= 10 && limit_day > 0) {
            info = QObject::tr("软件试用期还剩%1天，请输入验证密钥").arg(limit_day);
            if(1 == lds_messagebox::question(parent, MESSAGE_TITLE_VOID, info, QObject::tr("确定"), QObject::tr("继续"))) {
                return true;
            }
        }
        if(limit_day  <= 0) {
            info = QObject::tr("软件超过试用期，请输入验证密钥");
            if(1 == lds_messagebox::question(parent, MESSAGE_TITLE_VOID, info, QObject::tr("确定"), QObject::tr("取消"))) {
                return false;
            }
        }
...
void public_sql::drawTextForArabic(QPainter *painter, QRect rect, int align, const QString &text)
{
    ...
    if(ar_begin != -1) {
        ...
        if(align & Qt::AlignLeft) {
            ...
            r2 = QRect(r2.x(), r2.y(), r3.x() - r2.x(), r2.height());
        }
