...
QImage public_printby_ch_billno_over_view_data::getImage(const QString &logo_file_path)
{
    ...
    QFontMetrics fm(SONGTI_font);
    int yoff = paint(0);
    QRect paint_rect = QRect(0, 0, fm.width(QString(line_char_count, '-')) , yoff);

    double scle = PRINTER_80MM_WIDTH_PIXELS / fm.width(QString(PRINTER_80MM_WIDTH_CHARS, '-'));
    QImage image(paint_rect.size() * scle, QImage::Format_ARGB32);
    {
    ...
    if(false == logo.isNull()) {
        QImage image2(image.width(), image.height() + logo.height(), QImage::Format_ARGB32);
...
int public_printby_ch_billno_over_view_data::paint(QPainter *painter)
{
    QFontMetrics fontm(SONGTI_font);
    int x = 0;
    ...
    for(int k = 0, count = ins.count(); k < count; k++) {
        ...
        for(int k = 0, count = text_list.count(); k < count; k++ ) {
            ...
            if(painter) drawText(painter, QRect(x / xscale, y / yscale_o, fontm.width(text),  char_h * yscale_n / yscale_o ), align, text);
            if(k == count - 1) {//最后一个
                charaters += textCharaters(text) * xscale;
                x += fontm.width(text) * xscale;
...
QList<QByteArray> public_printby_ch_billno_over_view_data::textSplit_n_detect_width(int xoff, int xscale, const QByteArray &text)
{
    ...
    int text_index;
    for(int k = 0; k < text_list.count(); k ++) {
