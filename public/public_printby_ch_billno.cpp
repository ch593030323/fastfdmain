...
void public_printby_ch_billno::toPayPrint(const QString &bp_path, int bp_count, int label_flowid_greater_than, payPrintModes m)
{
    if(m & payDrawer) {
        openDrawer();
    ...
    if(m & payLabel) {
        public_printby_ch_billno::print_label(ch_billno, label_flowid_greater_than);
    }
}
...
void public_printby_ch_billno::makeIns(int paperwidth)
{
    ...
    if(can_print_sale_detail()
        ...
        for(int row = 0; row < model_bp_fmt->tablename_kvmap_count("sale_detail"); row ++) {
            ...
            bool ch_dish_group =
                    dish_printd->data(row, "ch_suitflag").toString() == "N" &&
                    dish_printd->data(row, "ch_addon_flag").toString() == "N"
                    ;
...
{
    QString preprint_text =  lds::localdataLocalePath + QString("/%1/print_receipt_preprint.txt").arg(cey_sys_parameterValue(model_bp_fmt, "locale_country_name").toString());
    if(QFile::exists(preprint_text)) {
        ...
        txt_ins_bill_print d(model.tablename_kvmap_d("bill_ins_table"), lds::fileOpenReadAll(preprint_text));
...
void public_printby_ch_billno::toprint_dish_return(const QString &path, const QStringList &int_flow_id_list)
{
    rowno = 0;
    //printpath
    print_format_init_path(path);
    ...
    //sale_detail
    model_bp_fmt_fill_orderdish_return(int_flow_id_list);
    //receipt_tail
    model_bp_fmt_fill_receipt_tail();
    //make ins
    makeIns(print_format.paperwidth);
...
{
    QString pay_text =  TXT_PRINT_RECEIPT_PAY_FILE_PATH(n_func::f_get_sysparm("locale_country_name"));
    if(QFile::exists(pay_text)) {
        lds_model_sqltablemodel model;
        translatorObj trObj(&model);
        trObj.setPrintTranslate(print_format.getLanguageKey(), this);
        ...
        txt_ins_bill_print d(model.tablename_kvmap_d("bill_ins_table"), lds::fileOpenReadAll(pay_text));
        print_format.print_ins = d.bill_before_reslove();
    } else {
...
QImage public_printby_ch_billno::toMakeFormImg()
{
    ...
    QImage img = d.getImage(logo_path);// /home/cz/图片/Lebanon.jpg
#ifdef QT_DEBUG
    img.save("print.png");
#endif
    return img;
...
void public_printby_ch_billno::toprintwaimai(const QString &path)
{
    ...
    qDebug() << "bp_fmt_person:" << model_bp_fmt_value("bp_fmt_person");
    //sale_detail
...
blockIns public_printby_ch_billno::toprint_preprint_div_tail(const QString &path, double subtotal, int idvcount)
{
    rowno = 0;
    print_format_init_path(path);
    qDebug() << model_bp_fmt_value("bp_fmt_cost") <<  idvcount;
...
void public_printby_ch_billno::model_bp_fmt_fill_orderdish_return(const QStringList &int_flow_id_list)
{
    lds_query_hddpos  query;
    //打印明细
    //打印本单序号、赠送标识、特价标识、超出长度截断、大类分组
    ...
    QString int_flow_ids = int_flow_id_list.join(",");
    QString dishname;
    QString dishname_2;
    dishname_split(bp_vch_dishname_No, dishname, dishname_2);
    //
    QString intersql =
            QString(
                " SELECT   " +
...
QSharedPointer<lds_model_sqltablemodel> public_printby_ch_billno::make_pay_detail_preprint(const lds_model_sqltablemodel *m, const QString &ch_billno, const QString &flowid_sql_filter)
{
    ...
    int tax_service_form_value = cey_sys_parameterValue(m, "tax_service_form_value", 0).toInt();
    //pay_receive
    ...
    ret_model->tablename_kvmap_insert_value("pay_detail", "num_amount", subtotal);
...
//int tax_service_form_value = cey_sys_parameterValue(model_bp_fmt, "tax_service_form_value", 0).toInt();
////pay_receive
//double pay_receive = tablemodel.getKeepdata().dish_total();
//double subtotal = w_scr_dish_pay_widget::valuerountbysys(pay_receive);//四舍五入
//double num_blotout = pay_receive - subtotal;
...
//ret_model->tablename_kvmap_insert_value("preprint_tip_detail", "tip_num_value_1",  tablemodel.getKeepdata().dish_total() * cey_sys_parameterValue(model_bp_fmt, "tip_int_rate_1").toDouble() / 100.0 );
//ret_model->tablename_kvmap_insert_value("preprint_tip_detail", "tip_num_value_2",  tablemodel.getKeepdata().dish_total() * cey_sys_parameterValue(model_bp_fmt, "tip_int_rate_2").toDouble() / 100.0 );
//ret_model->tablename_kvmap_insert_value("preprint_tip_detail", "tip_num_value_3",  tablemodel.getKeepdata().dish_total() * cey_sys_parameterValue(model_bp_fmt, "tip_int_rate_3").toDouble() / 100.0 );

//return subtotal;
//}

//        model_bp_fmt->tablename_kvmap_insert_value("pay_detail", "num_cost", tablemodel.getKeepdata().consum);
//        if(tablemodel.getKeepdata().discount != 0) {
//            model_bp_fmt->tablename_kvmap_insert_value("pay_detail", "num_discount",  tablemodel.getKeepdata().discount);
//        }
//        if(tablemodel.getKeepdata().present != 0) {
//            model_bp_fmt->tablename_kvmap_insert_value("pay_detail", "num_present",  tablemodel.getKeepdata().present);
//        }
//        if(tablemodel.getKeepdata().num_service != 0)//服务费 != 0
//            model_bp_fmt->tablename_kvmap_insert_value("pay_detail", "num_service",  tablemodel.getKeepdata().num_service);
//        if(tablemodel.getKeepdata().num_rate() != 0)//税率 != 0
//            model_bp_fmt->tablename_kvmap_insert_value("pay_detail", "num_rate",  tablemodel.getKeepdata().num_rate());
//        if(num_blotout != 0) //抹零 != 0
//            model_bp_fmt->tablename_kvmap_insert_value("pay_detail", "num_blotout",  num_blotout);
//    }
//    model_bp_fmt->tablename_kvmap_insert_value("pay_detail", "num_amount",  subtotal);

//    if(cey_sys_parameterValue(model_bp_fmt, "tip_enable").toString() == "1") {
//        model_bp_fmt->tablename_kvmap_insert_value("preprint_tip_detail", "tip_desc",  "Tip Guide");
//        model_bp_fmt->tablename_kvmap_insert_value("preprint_tip_detail", "tip_num_value_1",  tablemodel.getKeepdata().dish_total() * cey_sys_parameterValue(model_bp_fmt, "tip_int_rate_1").toDouble() / 100.0 );
//        model_bp_fmt->tablename_kvmap_insert_value("preprint_tip_detail", "tip_num_value_2",  tablemodel.getKeepdata().dish_total() * cey_sys_parameterValue(model_bp_fmt, "tip_int_rate_2").toDouble() / 100.0 );
//        model_bp_fmt->tablename_kvmap_insert_value("preprint_tip_detail", "tip_num_value_3",  tablemodel.getKeepdata().dish_total() * cey_sys_parameterValue(model_bp_fmt, "tip_int_rate_3").toDouble() / 100.0 );
//    }
...
{
    return getfixedlen(value, len, model_bp_fmt_value("bp_fmt_overstep_cut"));
}
...
void public_printby_ch_billno::txt_print_fill_sale_detail(lds_model_sqltablemodel *m, const QString &ch_billno)
{
    QStringList m_rowid;
...
*/
    ...
    query.execSelect(interSelectSql(m, false, "").arg(ch_billno) + " order by a.int_flowid asc ");
    while(query.next()) {
...
void public_printby_ch_billno::txt_print_bill_create_table(lds_model_sqltablemodel *m)
{
    ...
    txt_print_bill_create_field(m, "%xf", "pay_detail", "num_cost", "消费", "");//(m, query.recordValue("num_cost").toDouble(), cey_sys_parameterValue(m, "bp_fmt_amount_dec").toInt()));
    txt_print_bill_create_field(m, "%zs", "pay_detail", "num_present", "赠送", "");//(m, -1 * query.recordValue("num_present").toDouble(), cey_sys_parameterValue(m, "bp_fmt_amount_dec").toInt()));
    txt_print_bill_create_field(m, "%zk2", "pay_detail", "num_discount", "折扣", "");//(m, -1 * query.recordValue("num_discount").toDouble(), cey_sys_parameterValue(m, "bp_fmt_amount_dec").toInt()));
    txt_print_bill_create_field(m, "%fw3", "pay_detail", "num_service", "服务费", "");//(m, query.recordValue("num_service").toDouble(), cey_sys_parameterValue(m, "bp_fmt_amount_dec").toInt()));
    txt_print_bill_create_field(m, "%ss", "pay_detail", "num_rate", "税收", "");//(m, query.recordValue("num_rate").toDouble(), cey_sys_parameterValue(m, "bp_fmt_amount_dec").toInt()));
...
QString public_printby_ch_billno::interSelectSql(lds_model_sqltablemodel *m, bool pay_exit, const QString &and_sql_filter)
{
    QString dishname;
    QString dishname_2;
    dishname_split(cey_sys_parameterValue(m, "bp_vch_dishname_No", "vch_dishname").toString(), dishname, dishname_2);

    return QString(
                " SELECT " +
                dishname +" as dishname"
                ",  c.vch_unitname as unit"
                ",  (%1 - a.num_back) as num"//sum
                ",  a.num_price as price"
                ",  ( if(ch_presentflag = 'Y' ,   0 ,   (%1 - a.num_back)*a.num_price*0.01*a.int_discount )   ) as amount"//sum
                ",  (a.num_price*(%1 - a.num_back)*(1-a.int_discount*0.01) ) as disdetail"//sum
                ",  vch_print_memo"
            ...
            .arg(pay_exit?"0":"a.num_num")
            +"  where a.ch_billno = '%1' "+ (pay_exit?" and a.num_back > 0 " :"and (a.num_num - a.num_back) <> 0 ") + "and a.ch_dishno = b.ch_dishno and b.ch_unitno = c.ch_unitno "
            + and_sql_filter
            ;
}
...
{
    if(dishname_show.contains(",")) {
        dishname = dishname_show.split(",").value(0);
        dishname_2 = dishname_show.split(",").value(1);
    } else {
        dishname = dishname_show;
        dishname_2 = " '' ";
    }
}

void public_printby_ch_billno::payPrint(const QString &bp_ch_billno, const QString &bp_languageKey, int label_flowid_greater_than, int pay_print_mode, const QImage &image)
{
    ...
    if(bp_m & public_printby_ch_billno::payLabel) {
        public_printby_ch_billno::print_label(bp_ch_billno, label_flowid_greater_than);
    }
}
