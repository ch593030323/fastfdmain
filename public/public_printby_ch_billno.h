...
public:
    ...
    static void txt_print_fill_sale_detail(lds_model_sqltablemodel *m, const QString &ch_billno);
    ...
    static void payPrint(const QString &bp_ch_billno, const QString &bp_languageKey, int label_flowid_greater_than, int pay_print_mode,
                         const QImage &image);
...
public:
    ...
    void toPayPrint(const QString &bp_path, int bp_count, int label_flowid_greater_than, payPrintModes m);
    void toprint(const QString &path, int manual_bp_count = -1, payPrintMode pay_mode = payNormal);
    void toprint_dish_return(const QString &path, const QStringList &int_flow_id_list);
...
private:
    ...
    void model_bp_fmt_fill_orderdish_return(const QStringList &int_flow_id_list);
