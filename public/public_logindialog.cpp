...
{
    //信号连接
    connect(this,SIGNAL(signalheartbeatupdate()),this,SLOT(to_sql_heart_beat()));
    ...
    connect(ui->lineEdit_usr,SIGNAL(returnPressed()),ui->lineEdit_pwd,SLOT(setFocus()));
...
void public_loginDialog::to_sql_heart_beat()
{
    ...
    if(sqldt.isValid()) {//数据库定时备份
        QDateTime sqlbackupdt = QDateTime(sqldt.date(), lds::sysconf->value("w_sys_manage_sqlcopy_backup_strategy/sql_time_backup_value", QTime(10,0,0)).toTime());

        //开启定时备份&&时间范围内
        if(lds::sysconf->value("w_sys_manage_sqlcopy_backup_strategy/sql_time_backup", true).toBool()
                &&w_sys_manage_sqlcopy_backup_strategy::timeBackupRecords.find(sqlbackupdt) == w_sys_manage_sqlcopy_backup_strategy::timeBackupRecords.end()
                ) {
            if((sqldt.secsTo(sqlbackupdt)/60) == 0) {//相差一分钟
                w_sys_manage_sqlcopy_backup_strategy::timeBackupRecords.insert(sqlbackupdt, true);
        ...
        if(!lds_thread::getThreadDbAndCreat().open() ) {
...
void public_loginDialog::to_everytime_to_print_failed_kitchen_bill()//开始下载第一条漏打单
{
    if(lds::terminalCode == n_func::f_get_sysparm("kitchen_service_terminal")) {//当厨打服务ip和本机ip相同是才会厨打
        kitchenpolling->emitsignalTrigger();
        kitchen_print_polling->emitsignalTrigger();
    }
    //播放支付demo
...
bool public_loginDialog::connectdb_init_check(lds_messagebox_loading_show *loading)
{
    ...
    if(sql_isopen) {
...
#ifdef QT_DEBUG
#else
        //        query.execDirect(" set global wait_timeout=15;");
#endif
        ...
        //V3软件不能连接4.0以上的数据库
        query.execSelect(" select max(sqlversion) from cey_sys_sqlversion; ");
        query.next();
        if(query.recordValue(0).toString().compare("4.0") >= 0) {
            lds_messagebox::warning(this, MESSAGE_TITLE_VOID, QObject::tr("V3软件无法连接4.0或4.0以上的数据库"));
            return false;
        }
        //~V3软件不能连接4.0以上的数据库
#endif
        ...
        //主机时间和最后营业时间相差大于30天
        query.execSelect("select max(dt_operdate) from cey_u_checkout_master where ch_state = 'Y' ");
        if(query.next())
        {
        ...
        if(false == query.next()) {
            ...
            int_id ++;
            if(int_id > 255) {
                lds_messagebox::warning(this, MESSAGE_TITLE_VOID, QObject::tr("局域网最多支持255台机器"));
                return false;
            }
            query.execInsert("cey_sys_lan_terminal",
                             qrtVariantPairList()
                             << qrtVariantPair("vch_terminal", lds::terminalCode)
...
void public_loginDialog::threadInit()
{
    ...
    //    QObject().moveToThread(wxpolling);
    wxpolling->start();
    //    wxpolling->waitForStarted();
    //科传接口上传
    ttpolling = new w_scr_dish_TECH_TRANS_thread(this);
    //    QObject().moveToThread(ttpolling);
    ttpolling->start();
    //    ttpolling->waitForStarted();
    //厨房打印

    kitchenpolling = new w_bt_dish_kitchen_dish_thread(this);
    //    QObject().moveToThread(kitchenpolling);
    kitchenpolling->start();

    //    kitchenpolling->waitForStarted();

    //打印机状态判断
    kitchen_print_polling = new w_bt_dish_kitchen_dish_print_state_thread(this);
    //    QObject().moveToThread(kitchen_print_polling);
    //    kitchen_print_polling->start();
    //    kitchen_print_polling->waitForStarted();
    //判断云端和本地版本号差异
    yun_version_diff_polling = new w_sys_manage_cloudsync_with_time_check_state_thread(this);
    //    QObject().moveToThread(yun_version_diff_polling);
    yun_version_diff_polling->start();
    //    yun_version_diff_polling->waitForStarted();
    //流水上传
    yun_flow_upload_polling = new w_sys_manage_cloudsync_with_time_flow_upload_thread(this);
    //    QObject().moveToThread(yun_flow_upload_polling);
    //星摩尔接口
    isoft_upload_polling = new business_data_upload_isoft_thread(this);
    isoft_upload_polling->start();
    //    isoft_upload_polling->waitForStarted();
    //丰泽接口
    fengze_upload_polling = new w_sys_manage_outer_pay_set_fengze_thread(this);
    fengze_upload_polling->start();
    //    fengze_upload_polling->waitForStarted();
    //实时上传-基础资料
    keepdata_time_basedata_polling = new w_sys_manage_cloudsync_with_time_basedata_thread(this);
    keepdata_time_basedata_polling->start();
    //    keepdata_time_basedata_polling->waitForStarted();
    //结算线程打印
    qt_recipt_print_polling = new w_bt_dish_Kitchenprinter_printer_thread(this);
    //    QObject().moveToThread(qt_recipt_print_polling);
    qt_recipt_print_polling->start();
    //    qt_recipt_print_polling->waitForStarted();
    //客显
    qt_display_print_polling = new w_bt_dish_Kitchenprinter_display_thread(this);
    //QObject().moveToThread(qt_display_print_polling);
    qt_display_print_polling->start();
    //    qt_display_print_polling->waitForStarted();
    //客显
    tel_box_polling = new TelBox_thread(this);
    tel_box_polling->start();
    //    tel_box_polling->waitForStarted();
    //音乐

    ogg_play_polling = new lds_ogg_play_thread(this);
    ...
    //辽宁顶有鸣
    liaoning_inventory_keep_polling = new w_scr_dish_liaoning_inventory_keep_thread(this);
    liaoning_inventory_keep_polling->start();
    liaoning_inventory_keep_polling->waitForStarted();
    ///
...
void public_loginDialog::runFrameupdate()
{
    ...
    if(w
            && QString("w_scr_dish_restaurant_dialog") == w->metaObject()->className())
    {
        value = runstep%(runRestaurantTimeout+1);

        if(value == runRestaurantTimeout) {
            emit signalrestaurantupdate();
        }
