...
w_scr_dish_pay_widget::w_scr_dish_pay_widget(w_scr_dish_pay_widget::operatModes m, QWidget *parent, const fexpandmain_model_sqltablemodel_data *tablemodel, int int_flowid_greater_than)
    : QFrame(parent)
{
    initGeometry(m);
    initdataByM(tablemodel, int_flowid_greater_than);
}
...
void w_scr_dish_pay_widget::initdataByM(const fexpandmain_model_sqltablemodel_data *tablemodel, int int_flowid_greater_than)
{
    this->int_flowid_greater_than =int_flowid_greater_than;
...
void w_scr_dish_pay_widget::refreshControl()
{
    ...
    ui->lineEdit_int_rate->setValue(ui->lineEdit_tax_cur->value() + ui->lineEdit_tax_cur_2->value() + ui->lineEdit_tax_cur_3->value());

    pay_keep_data.print();

    //v
    int tax_service_form_value = model_orderdish->get_cey_sys_parameter_value("tax_service_form_value", 0).toInt();
    ...
    //低消
    ui->lineEdit_service->setValue(pay_keep_data.pay_service(model_orderdish->cur_ch_billno()));
    ui->lineEdit_service->setMessage(pay_keep_data.pay_service_desc(model_orderdish->cur_ch_billno()));
...
*/
    ...
    lds_query_hddpos  query;
    lds_query_hddpos::tran_saction();
    toMPay(dt, errstring, ch_payno, serial_no_cur);
...
*/lds_query_hddpos::com_mit();
        ...
        //        printLabel(model_orderdish->cur_ch_billno());
        //~打印标签
        //        while(1 )
        //#ifdef QT_DEBUG
        if(w_sys_manage_outer_pay_set_locale_wizard::bp_country_print_img()) {
            ...
            public_sql::save_login->qt_recipt_print_polling->emitsignalTrigger(model_orderdish->cur_ch_billno(),
                                                                               bp_languageKey, int_flowid_greater_than, pay_print_mode,
                                                                               image);
        } else {
            QString bp_languageKey = lds::sysconf->value("system_setting/maidan_language").toString();
            public_sql::save_login->qt_recipt_print_polling->emitsignalTrigger(model_orderdish->cur_ch_billno(),
...
void w_scr_dish_pay_widget::toMPay(const QDateTime &dt, QString &errstring, QString &ch_payno, const QString &serial_no_cur)
{
    ///预付款插入
    lds_query_hddpos  query;
    bool gonext = true;
    if(gonext) {
        ...
        bool kuaican_quit = pay_operate_mode & OPERAT_QUITBILL
                ...
                ;
        gonext = model_orderdish->try_commit_data(errstring, kuaican_quit);
        if(gonext) {
            if(kuaican_quit) {
...
void w_scr_dish_pay_widget:: payAndYunCommit(const QDateTime &dt, QString &errstring, QString &ch_payno, QString serial_no_cur)
{
    lds_query_hddpos  query;
    QString pay_datetime_str = dt.toString("yyyy-MM-dd hh:mm:ss");
    member_up_yun muy;
    ...
    //ch_payno
    ch_payno = lds_scr_table::get_max_ch_payno_add1(dt);
...
void w_scr_dish_pay_widget::dataupload(const QString &ch_payno, const QDateTime &dt)
{
    ...
    if(n_func::f_get_sysparm("fengze_interface") == "1") {
        ...
        public_sql::save_login->fengze_upload_polling->emitsignalTrigger();
    }
    //~丰泽
}
...
void w_scr_dish_pay_widget::resetPaySection()
{
    ...
    qDebug() << "pay_section_record" << pay_section_record;
    for(int k = 0; k < pay_section.count(); k++) {
        QString p = pay_section_record.value(k);
...
{
    int tax_service_form_value = model_orderdish->get_cey_sys_parameter_value("tax_service_form_value", 0).toInt();
    return fexpandmain_model_sqltablemodel::pay_without_low_service(tax_service_form_value,
