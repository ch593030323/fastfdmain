...
public:
    ...
    w_scr_dish_pay_widget(w_scr_dish_pay_widget::operatModes m, QWidget *parent,
                          const fexpandmain_model_sqltablemodel_data *tablemodel, int int_flowid_greater_than = -1);

    void watchControl();

    ~w_scr_dish_pay_widget();
    void initdataByM(const fexpandmain_model_sqltablemodel_data *tablemodel, int int_flowid_greater_than = -1);
...
private:
    void toMPay(const QDateTime &dt, QString &errstring , QString &ch_payno, const QString &serial_no_cur);//买单
    bool otherModePay(const QString &ch_paymodeno, double amount = 0, const QString & wtitle = "");
    /*付款时本地云端提交*/
    void payAndYunCommit(const QDateTime &dt, QString &errstring , QString &ch_payno, QString serial_no_cur);
    /*营业数据上传和打印*/
    void dataupload(const QString &ch_payno, const QDateTime &dt);
    /*打印*/
    void printLabel(const QString &ch_billno);
    qlonglong int_flowid_greater_than;
