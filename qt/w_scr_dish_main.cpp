...
void w_scr_dish_main::torefresh_dish_sql()
{
    ...
    QString dishname;
    QString dishname_2;
    public_printby_ch_billno::dishname_split(lds::sysconf->value("w_scr_dish_main/dishnameswitch", "vch_dishname").toString(), dishname, dishname_2);

    model_dish->sql_cur =
            "  select "
            "  concat(" + QString(" ifnull(%1,'') ").arg(dishname) + " , '\n' , " + "b.num_price) as name_price" //描述
...
void w_scr_dish_main::loadData()
{
    ...
    lds::hideWidget(frame_dish_bottom2->desk);
    lds::hideWidget(frame_dish_bottom2->waimai);
    lds::hideWidget(frame_dish_bottom2->waidai);
#endif
...
void w_scr_dish_main::toquitdish2()
{
    ...
    QList<num_backData> list;
    QString ch_billno = tablemodel->cur_ch_billno();

    dialog.row_data_map_print();
    dialog.row_data_map2List(vch_reason, list);
    lds_query_hddpos::tran_saction();
    foreach(const num_backData &d, list) {
        ...
        int_flow_id_list << QString::number(d.int_flowid);
    }
    if(!query.execUpdate("cey_u_table","num_cost", 0, qrtEqual("ch_billno", ch_billno))) {
        lds_query_hddpos::roll_back();
    ...
    printer.toprint_dish_return(printer.defpath(), int_flow_id_list);
}
...
{
    if(fexpandmain_2::rejected != fexpandmain_2::to_m_read(this, tablemodel)) {
        updatememberchecked();
    }
    //2
}
...
void w_scr_dish_main::tosyssuoping()
{
    ...
    dialog.setWindowTitle(QObject::tr("前台锁屏"));
    transparentCenterDialog(&dialog).exec();
}
...
void w_scr_dish_main::todishclick(const QString &ch_dishno)
{
    ...
    //
    foreach(const QString &dishno, dishno_list) {
        //负库存不允许销售
...
void w_scr_dish_main::toTelComing(const QString &telNo)
{
    ...
    if(aw && QString("w_scr_dish_main_view_800x600") == aw->metaObject()->className()) {
        ...
        if(QDialog::Accepted == transparentCenterDialog(&dialog).exec()) {
            if(fexpandmain_2::readed == fexpandmain_2::to_m_read(this, tablemodel, dialog.ret_vch_memberno)) {
...
bool w_scr_dish_main::printdish_tran_saction(QString &errstring)
{
    ...
    if(is_need_insert) {
        ...
        if(lds::sysconf->value("system_setting/luodanzongdan").toString() == "1") {
            ...
            printer.toprint_preprint(lds::sysconf->value("system_setting/printerpath").toString(), QObject::tr("消费单"), QString(" int_flowid > %1 ").arg(print_int_flowid_max));
        }//success
        //标签
