...
void w_scr_dish_restaurant_dialog::torefresh(int cur, int total)
{
    ...
    modeltable->sql_cur = modeltable->sql_cur
            ...
            ;
    modeltable->sqlother = modeltable->sql_cur + " where ifnull(ch_areano, '') like '%1%' ";
    modeltable->sql_cur = modeltable->sqlother.arg(modeltable->key_sqlother);
...
void w_scr_dish_restaurant_dialog::tolockscreen()
{
    ...
    lock.setWindowTitle(QObject::tr("前台锁屏"));
...
{
    return  " select  '"+QObject::tr("所有区域")+"' as vch_areaname, '%' as ch_areano union SELECT concat(vch_areaname,'[',ch_areano,']') as nametype,  ch_areano  FROM cey_bt_table_area ";
}
