...
class lds_model_sqltablemodel : public QSqlTableModel
{
    Q_OBJECT
public:
    enum ModelOpera{
...
public:
    lds_model_sqltablemodel(QObject *parent = 0);
    virtual ~lds_model_sqltablemodel();
    ...
    void appendTableRow(const lds_model_sqltablemodel *model, int row);
...
private:
    ...
    QString flags_editable_by_column_fieldname;
...
private:
    ...
    QSet<QString> SELECT_NOT_CLEAR_MAP_KEY_set;//当map在select中清空时，这些需要保留
...
private:
    void tablename_kvmap_delete_all();
...
Q_DECLARE_OPERATORS_FOR_FLAGS(lds_model_sqltablemodel::ModelOperas)
