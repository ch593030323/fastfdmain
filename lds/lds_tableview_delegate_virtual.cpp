...
#define NumLayoutAlignRight (qApp->layoutDirection() == Qt::RightToLeft ? Qt::AlignLeft : Qt::AlignRight)
#define NumLayoutAlignLeft (qApp->layoutDirection() == Qt::RightToLeft ? Qt::AlignRight : Qt::AlignLeft)
    ...
    QStyledItemDelegate(parent)
{
    ...
    bp_fmt_price_dec = lds_query_hddpos::selectValue(query, " select vch_value from cey_sys_parameter where vch_parameter = 'bp_fmt_price_dec' ").toInt();
    bp_fmt_num_dec = lds_query_hddpos::selectValue(query, " select vch_value from cey_sys_parameter where vch_parameter = 'bp_fmt_num_dec' ").toInt();
    bp_fmt_amount_dec = lds_query_hddpos::selectValue(query, " select vch_value from cey_sys_parameter where vch_parameter = 'bp_fmt_amount_dec' ").toInt();
...
//        painter_text_rect.adjust(opt.decorationSize.width() + text_spacing, 0, 0, 0);
    }
    if(lds::isNumber(dd)) {
...
{
    if(false == b_paint_transfer) return dd.toString();
