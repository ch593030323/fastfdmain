...
const QString lds_model_sqltablemodel::SELECT_NOT_CLEAR_MAP_KEY = "SELECT_NOT_CLEAR_MAP_KEY";
const QString lds_model_sqltablemodel::SELECT_AUTO_REFRESH_KEY = "SELECT_AUTO_REFRESH_KEY";
...
lds_model_sqltablemodel::lds_model_sqltablemodel(QObject *parent)
    : QSqlTableModel(parent, lds_query_thread::getThreadDbAndOpen())
{
    select_count = 0;
    tablename_enable_query_enable = true;
}

lds_model_sqltablemodel::~lds_model_sqltablemodel()
{
    tablename_kvmap_del();
}
...
Qt::ItemFlags lds_model_sqltablemodel::flags(const QModelIndex &index) const
{
...
lds_model_sqltablemodel::ModelOpera lds_model_sqltablemodel::opera(int row) const
{
...
void lds_model_sqltablemodel::enable_vheaderdata_dirty_translate()
{
    b_enable_vheaderdata_dirty_translate = true;
}

void lds_model_sqltablemodel::enable_only_edited_when_OpInsert(int column)
{
    set_only_edited_when_OpInsert.insert(column);
}

void lds_model_sqltablemodel::setFilter(const QString &filter)
{
...
void lds_model_sqltablemodel::setLastData(const QString &field_name, const QVariant &value, int role)
{
...
void lds_model_sqltablemodel::appendTableRow(const lds_model_sqltablemodel *model, int row)
{
    int k = this->rowCount();
    qDebug() << this->rowCount();
    qDebug() <<this->insertRow(k);
    qDebug() << this->rowCount();
...
double lds_model_sqltablemodel::model_data_NIBOLAN(int row, const QVector<QString> &expression) const
{
...
//尼波兰
//若是非表达式，就会崩溃
double lds_model_sqltablemodel::model_data_NIBOLAN(int row, const QString &expression) const
{
    return model_data_NIBOLAN(row, lds_expression2RPN::get_rpn_by(expression));
}

QString lds_model_sqltablemodel::model_expression_replace(const QString &expression, const QString &keyOld, const QString &keyNew) const
{
    ...
    for(int k = 0; k < str.count(); k++) {
        if(str.mid(k, keyOld.length()) == keyOld) {
            if((k + keyOld.length()) >= str.length()
                    || str.mid(k + keyOld.length(), 1).contains(QRegExp("[()+\\-*/=,]"))) {
...
double lds_model_sqltablemodel::model_data_IF(int row, const QString &expression) const
{
...
double lds_model_sqltablemodel::model_data_IF_equalY(int row, const QString &expression) const
{
...
double lds_model_sqltablemodel::model_data_IF_equalY(int row, const QString &condition, const QString &expression1, const QString &expression2) const
{
...
double lds_model_sqltablemodel::model_data_IF_equalY(int row, const QString &condition, const QVector<QString> &expression1, const QVector<QString> &expression2) const
{
    QVariant c1, c2;
...
bool lds_model_sqltablemodel::model_data_IF_equalY_reslove(const QString &expression, QString &condition, QString &expression1, QString &expression2) const
{
...
//if(a=b,c,d)
//if(a=b,c,(e+4)*d)
bool lds_model_sqltablemodel::model_data_IF_equalY_format(const QString &expression, QString *errstring) const
{
    QString p = expression;
    p = p.replace(" ", "");
...
QVariant lds_model_sqltablemodel::model_data(int row, const QString &field_name, int role) const
{
#ifdef QT_DEBUG
...
QVariant lds_model_sqltablemodel::model_data(const QModelIndex &index, int role) const
{
    return model_data(index.row(), index.column(), role);
}

QVariant lds_model_sqltablemodel::model_data(int row, int column, int role) const
{
    return this->data(this->index(row, column), role);
}

bool lds_model_sqltablemodel::model_data_set(const QModelIndex &index, const QVariant &value, int role)
{
    return setData(index,  value, role);
}

bool lds_model_sqltablemodel::model_data_set(int row, const QString &field_name, const QVariant &value, int role)
{
#ifdef QT_DEBUG
    if(debug_model_crash_when_field_not_valid && this->fieldIndex(field_name) < 0) {
...
bool lds_model_sqltablemodel::model_data_set(int row, int column, const QVariant &value, int role)
{
    return model_data_set(this->index(row, column), value, role);
}

QModelIndex lds_model_sqltablemodel::model_index(int row, const QString &field_name)
{
    return this->index(row, this->fieldIndex(field_name));
}

QModelIndex lds_model_sqltablemodel::model_index(int row, int column)
{
    return this->index(row, column);
}

bool lds_model_sqltablemodel::model_is_dirty() const
{
    return rowCountOp(OpDirty) > 0;
}

bool lds_model_sqltablemodel::model_is_dirty(int row) const
{
    return opera(row) != OpNone;
}

bool lds_model_sqltablemodel::model_find(const QString &field_name, const QString &cmp_value)
{
...
QString lds_model_sqltablemodel::max_column_add_1_bysql(const QString &field_name, const char *cformat, int min)
{
    return QString().sprintf(cformat, max_column_add_1_bysql(field_name, min));
}

int lds_model_sqltablemodel::max_column_add_1_bysql(const QString &field_name, int min)
{
...
int lds_model_sqltablemodel::max_column_add_1_dec(const QString &field_name, int min)
{
    int value=0;
    int cmp_value;
    for(int r=0,r_count=this->rowCount();r<r_count;r++) {
...
QString lds_model_sqltablemodel::max_column_add_1__max_sql_add_1_dec(const QString &field_name, const char *cformat, int min)
{
    return QString().sprintf(cformat, max_column_add_1__max_sql_add_1_dec(field_name, min));
}

int lds_model_sqltablemodel::max_column_add_1__max_sql_add_1_dec(const QString &field_name, int min)
{
    return qMax(max_column_add_1_bysql(field_name, min),
                max_column_add_1_dec(field_name, min));
}

void lds_model_sqltablemodel::enable_debug_model_not_crash_when_field_not_valid()
{
#ifdef QT_DEBUG
...
void lds_model_sqltablemodel::save_set_header_clear()
{
    save_set_header_saver_column_list.clear();
    save_set_header_can_edited.clear();
}

bool lds_model_sqltablemodel::save_set_header(const QString &field_name, int iseditable)
{
    return save_set_header(this->fieldIndex(field_name), iseditable);
}

bool lds_model_sqltablemodel::save_set_header(int section, int iseditable)
{
    //header save
...
bool lds_model_sqltablemodel::save_set_header(int section, const QString &hheaderStr, int iseditable)
{
    save_section_value_map.insert(section, hheaderStr);
    return save_set_header(section, iseditable);
}

bool lds_model_sqltablemodel::save_set_header(const QString &field_name, const QString &hheaderStr, int iseditable)
{
    return save_set_header(fieldIndex(field_name), hheaderStr, iseditable);
}

QList<int> lds_model_sqltablemodel::get_save_set_header_saver_column_list()
{
    return save_set_header_saver_column_list;
}

bool lds_model_sqltablemodel::select()
{
    if(!setFilter_select)
...
double lds_model_sqltablemodel::get_value_by_nibolan(const QVector<QString> &nibolan_str, int row) const
{
    QStack<double> stack;
    bool ok;
    double p0;
...
QVariant lds_model_sqltablemodel::data(const QModelIndex &idx, int role) const
{
    QVariant val;
    if(role < Qt::UserRole)
...
bool lds_model_sqltablemodel::setData(const QModelIndex &index, const QVariant &value, int role)
{
...
bool lds_model_sqltablemodel::submitAll()
{
    return QSqlTableModel::submitAll();
}

QVariant lds_model_sqltablemodel::headerData(int section, Qt::Orientation orientation, int role) const
{
...
int lds_model_sqltablemodel::rowCountOp(ModelOperas ops) const
{
...
bool lds_model_sqltablemodel::submitRow(int row, QString *errstring)
{
    lds_query_db query(this->database());
...
QSqlError lds_model_sqltablemodel::lastError() const
{
    return QSqlTableModel::lastError();
}

bool lds_model_sqltablemodel::trySubmitAll(QString *errstring)
{
    //insert
...
bool lds_model_sqltablemodel::trySubmitAllUpdateLoading(QString *errstring, QObject *object, void (*updateProgress)(QObject *, qint64, qint64))
{
    //bytesTotal
    qint64 bytesStep = 0;
...
bool lds_model_sqltablemodel::removeRows(int row, int count, const QModelIndex &parent)
{
...
bool lds_model_sqltablemodel::insertRows(int row, int count, const QModelIndex &parent)
{
    //
    for(int k = 0; k < count; k++) {
...
void lds_model_sqltablemodel::SELECT_AUTO_REFRESH_refresh(const QString &tablename) const
{
    //d
    SELECT_AUTO_REFRESH_data  sqls = SELECT_AUTO_REFRESH_map.value(tablename);
    lds_model_sqltablemodel_d* d =  tablename_kvmap.value(tablename, 0);
    if(0 == d) {
        tablename_kvmap_insert_table(tablename);
        d = tablename_kvmap.value(tablename);
    }
    //key_value_sql
...
void lds_model_sqltablemodel::tablename_kvmap_insert_table(const QString &tablename) const
{
    //1
    lds_model_sqltablemodel_d *d = tablename_kvmap.value(tablename, 0);
    if(0 == d) {
        ( (QMap<QString/*tablename*/, lds_model_sqltablemodel_d *> *)(&tablename_kvmap) )
                ->insert(tablename, new lds_model_sqltablemodel_d);
    }
}

void lds_model_sqltablemodel::tablename_kvmap_insert_value(const QString &tablename, const QString &key, const QVariant &value) const
{
    tablename_kvmap_insert_field(tablename, key, "", value);
}

void lds_model_sqltablemodel::tablename_kvmap_insert_field(const QString &tablename, const QString &key, const QString &field_name, const QVariant &field_value) const
{
    //1
    if(SELECT_NOT_CLEAR_MAP_KEY_set.find(tablename) == SELECT_NOT_CLEAR_MAP_KEY_set.end()) {
        ( ( QSet<QString> *)(&SELECT_NOT_CLEAR_MAP_KEY_set) )
                ->insert(tablename);
    }
    //2
    tablename_kvmap_insert_table(tablename);
    //3
    tablename_kvmap.value(tablename)->setData(key, field_name, field_value);//->insert(key, value);
}

void lds_model_sqltablemodel::tablename_kvmap_insert_field(const QString &tablename, const QString &key, const QVariantMap &map) const
{
...
void lds_model_sqltablemodel::tablename_kvmap_insert_field(const QString &tablename, const QString &key, const qrtVariantPairList &pairs) const
{
...
void lds_model_sqltablemodel::tablename_kvmap_insert_field(const QString &tablename, const QString &key, const QSqlRecord &record) const
{
...
void lds_model_sqltablemodel::tablename_kvmap_insert_sql(const QString &tablename, const QString &sql_keyname, const QString &sql_valuename, const QString &sql_filter, bool firstEmpty, const QString &SELECT_AUTO_REFRESH_KEY)
{
...
void lds_model_sqltablemodel::tablename_kvmap_insert_sql(const QString &tablename, const QString &key_value_sql, bool firstEmpty, const QString &SELECT_AUTO_REFRESH_KEY)
{
...
QVariant lds_model_sqltablemodel::sql_value(const QString &tablename, const QString &sql_keyvalue, const QString &sql_keyname, const QString &sql_valuename, const QString &sql_filter) const
{
...
void lds_model_sqltablemodel::tablename_kvmap_delete_all()
{
    QMapIterator< QString/*tablename*/, lds_model_sqltablemodel_d *>  map_itr(tablename_kvmap);
    while(map_itr.hasNext()) {
        map_itr.next();
        tablename_kvmap_delete(map_itr.key());
    }
}

const lds_model_sqltablemodel_d *lds_model_sqltablemodel::tablename_kvmap_d(const QString &tablename) const
{
    return tablename_kvmap.value(tablename, 0);
}

void lds_model_sqltablemodel::tablename_kvmap_print() const
{
    QMapIterator<QString/*tablename*/, lds_model_sqltablemodel_d *> i(tablename_kvmap);
    while (i.hasNext()) {
        i.next();
        qDebug() << "============== == " << i.key() << "============== == ";
        i.value()->print();
...
void lds_model_sqltablemodel::tablename_kvmap_print(const QString &tablename) const
{
...
bool lds_model_sqltablemodel::tablename_key_existed(const QString &tablename, const QString &key) const
{
    const lds_model_sqltablemodel_d *d = tablename_kvmap_d(tablename);
    if(d != 0) {
        return d->getMapRowstr().find(key) != d->getMapRowstr().end();
    }
    return false;
}

void lds_model_sqltablemodel::tablename_enable_query(bool enable)
{
    tablename_enable_query_enable = enable;
}

int lds_model_sqltablemodel::tablename_kvmap_count(const QString &tablename) const
{
    const lds_model_sqltablemodel_d *d = tablename_kvmap_d(tablename);
    if(d != 0) {
        return d->rowCount();
...
QString lds_model_sqltablemodel::tablename_kvmap_indexof(const QString &tablename, const qrtVariantPairList &select_arg, bool *ok) const
{
    const lds_model_sqltablemodel_d *d = tablename_kvmap_d(tablename);
    int row = -1;
    if(ok) *ok = false;
...
void lds_model_sqltablemodel::tablename_kvmap_delete(const QString &tablename) const
{
    if(0 != tablename_kvmap_d(tablename)) {
        ((QMap<QString/*tablename*/, lds_model_sqltablemodel_d *> *)(&tablename_kvmap))
                ->find(tablename).value()->clear();
    }
}

void lds_model_sqltablemodel::tablename_kvmap_drop(const QString &tablename) const
{
    if(0 != tablename_kvmap_d(tablename)) {
        ((QMap<QString/*tablename*/, lds_model_sqltablemodel_d *> *)(&tablename_kvmap))->remove(tablename);
    }
}

void lds_model_sqltablemodel::tablename_kvmap_delete_for_select_and_autoselect()
{
    QMapIterator< QString/*tablename*/, lds_model_sqltablemodel_d *>   map_itr(tablename_kvmap);
    while(map_itr.hasNext()) {
        map_itr.next();
        if(SELECT_NOT_CLEAR_MAP_KEY_set.contains(map_itr.key()))
            continue;
        tablename_kvmap_delete(map_itr.key());
    }
...
QVariant lds_model_sqltablemodel::tablename_kvmap_value(const QString &tablename, const QString &sql_keyvalue, const QString &sql_keyname, const QString &sql_valuename, const QString &sql_filter, const QVariant &default_value) const
{
    if(tablename_kvmap.value(tablename, 0) == 0) {
        tablename_kvmap_insert_table(tablename);
    }
    const lds_model_sqltablemodel_d *d = tablename_kvmap_d(tablename);
...
QVariant lds_model_sqltablemodel::tablename_kvmap_value(const QString &tablename, const QString &key, const QVariant &default_value) const
{
    return tablename_kvmap_field(tablename, key, "", default_value);
}

QVariant lds_model_sqltablemodel::tablename_kvmap_field(const QString &tablename, const QString &key, const QString &field_name, const QVariant &default_value) const
{
    const lds_model_sqltablemodel_d *d = tablename_kvmap_d(tablename);
    if(0 != d) {
        return d->data(key, field_name);
    }
    return default_value;
}

qrtVariantPair lds_model_sqltablemodel::tablename_kvmap_pair(const QString &tablename, const QString &key, const QString &field_name) const
{
    const lds_model_sqltablemodel_d *d = tablename_kvmap_d(tablename);
    if(0 != d) {
...
void lds_model_sqltablemodel::tablename_kvmap_del()
{
    QMapIterator< QString/*tablename*/, lds_model_sqltablemodel_d *>  map_itr(tablename_kvmap);
    while(map_itr.hasNext()) {
        map_itr.next();
        delete map_itr.value();
    }
    tablename_kvmap.clear();
}

//只记录，非插入的数据
void lds_model_sqltablemodel::insert_row_where_sql_map(int row)
{
    QString sql;
...
void lds_model_sqltablemodel::insert_row_where_sql_map(int row, const QString &sql)
{
...
QString lds_model_sqltablemodel::insert_row_where_sql(int row)
{
    QString where_sql;
    for(int p = 0; p < primaryKey().count(); p ++) {
        QString kname = primaryKey().fieldName(p);
