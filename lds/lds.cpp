...
lds_const_var<QString> lds::localdataLocalePath = lds::localdataPath + "/locale";
lds_const_var<QString> lds::arg_d_value;

lds_const_int lds::SYS_QRCODE_ORDER_PORT = 4646;

QSettings * lds::sysconf = 0;

static lds::resultCode  getTerminalCode(QString &terminalCode, QString *errstring = 0);

lds::resultCode lds::LDS_INIT(QString *errstring)
{
    ...
    if(screen_primary_size.contains("x")) {
        lds::FULL_WINDOW_SIZE.setOther(QSize(screen_primary_size.split("x").value(0).toInt(), screen_primary_size.split("x").value(1).toInt()));
    } else {
        lds::FULL_WINDOW_SIZE.setOther(qApp->desktop()->availableGeometry().size());
#ifdef Q_OS_WIN
    ...
    //arm 且 发布
#elif defined(Q_OS_UNIX) && defined(QT_NO_DEBUG)
...
QString lds::reportModelMapValue(const QVariant &dd, const QString &defaultValue)
{
    if(reportModelMap.isEmpty()) {
        ...
        reportModelMap.insert("ch_dish_typeno", QObject::tr("大类"));
...
QStringList lds::get_fastfdbacktext()
{
    ...
    if(!fastfdbacktext.exists()) {
        ...
        fastfdbacktext.write(QObject::tr("摇钱数软件").toLocal8Bit());
...
//    terminalCode =  "AA80801131630036C";
    ...
    //    if(false == lds::dogLoad(terminalCode, errstring)) {
    ...
    //        //若是没有激活的话
    //        int remainday = settings.value("window_soft_limit/date").toDate().daysTo(cur_date);
    //        if(remainday > 30) {
    //            if(errstring)
    //                *errstring = QObject::tr("软件使用已经超过30天，请插入加密狗以便继续使用");
    //            return lds::resultError;
    //        } else {
    //            //软件在使用期内
    //            if(errstring)
    //                *errstring = QObject::tr("您的软件到期时间剩余%1天").arg(30 - remainday);
    //        }
    //        terminalCode = "TEST_TERMINAL";
    //        return lds::resultInfo;
