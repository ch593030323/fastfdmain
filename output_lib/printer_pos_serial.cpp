...
{
    if(SERIAL_MAP.map.value(port, 0) == 0 ) {
        QStringList list = port.split(",");
        QString path = list.value(0).trimmed();
        SERIAL_MAP.map.insert(port, new QextSerialPort);
        SERIAL_MAP.map.value(port)->setPortName(path);
    }
    return SERIAL_MAP.map.value(port);
}

void Printer_POS_SERIAL::SERIAL_PORT_MUTEX_COM_MAP_CLOSE_ALL()
{
    QMapIterator<QString, QextSerialPort *> mapitr(SERIAL_MAP.map);
...
Printer_POS_SERIAL::SERIAL_PORT_MUTEX_COM_MAP::~SERIAL_PORT_MUTEX_COM_MAP()
{
    for(QMap<QString, QextSerialPort *>::iterator k = map.begin(); k != map.end(); k++) {
        k.value()->close();
        delete k.value();
    }
    map.clear();
